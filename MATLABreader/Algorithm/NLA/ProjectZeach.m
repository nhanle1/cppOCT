...
outputdir=[cd,'\',folder];mkdir(outputdir);
...
if do_NM
    for iY=1:nY
        BLn(:,:,iY)     =NLnorm(BL(:,:,iY),STR(:,:,iY),5,5,0.1);
    end
    RgFlowNM_ed=prctile(double(BLn(1:56:end)),[0 100]);
...
if do_proj
    ...
    %     IMF=squeeze(BLn);
    %     IMS=log1p(double(IMF(ind)))/65535;
    %     Rg=prctile(IMS(1:20:end),[1 100]);
    ...
    for i=1:1:Info.nZseg-5
        temp    =mat2gray(squeeze(median(IMS(i:i+2,:,:),1)));
        Rg=prctile(temp(1:17:end),[60 100]);
