function [IMGout,Dx] = OCTA_F_SubPixReg(IMGin,Nsub,Colshift)
			%Nsub=10; Colshift=0;
            IMGout=IMGin;
			[nZcrop,nX,nR]=size(IMGin);
            Dx=zeros(1,nR);
            Dy=Dx;
            f=abs(IMGin(:,:,1));% Reference Image
            for iR=2:nR
                g=abs(IMGin(:,:,iR));% Struct image
                f1=abs(angle(IMGin(:,:,iR)));
                [output,Greg,~] = OCTA_F_dftreg(fft2(f),fft2(g),fft2(f1),Nsub,Colshift);%DFT Kernel
                Dx(iR)=output(3);
                Dy(iR)=output(4);           
                f=abs(ifft2(Greg));% Upadate ref image
            end
            %%  apply shift     
            Dy=Dy*0;
            fftScale = round(Nsub/2);
            DxUp=round((-Dx*fftScale));
            DyUp=round((-Dy*fftScale));
            IMGinUp = fft(ifft(IMGin,nZcrop,1),nZcrop*fftScale,1);
            nZcropUp=nZcrop*fftScale;
            As=zeros(nZcropUp,nX);
            for iR=2:nR
                A=IMGinUp(:,:,iR);dx=DxUp(iR);dy=DyUp(iR);
                if (dx>0)&&(dy>=0)
                    As(1:nZcropUp-dx,1:nX-dy)=A(dx+1:nZcropUp,dy+1:nX);
                    IMGout(:,:,iR)=As(1:fftScale:end,:);
                elseif (dx<0)&&(dy<=0)
                    As(-dx+1:nZcropUp,-dy+1:nX)=A(1:nZcropUp+dx,1:nX+dy);
                    IMGout(:,:,iR)=As(1:fftScale:end,:);
                elseif (dx>0)&&(dy<0)
                    As(1:nZcropUp-dx,-dy+1:nX)=A(dx+1:nZcropUp,1:nX+dy);
                    IMGout(:,:,iR)=As(1:fftScale:end,:);
                elseif (dx<0)&&(dy>0)
                    As(-dx+1:nZcropUp,1:nX-dy)=A(1:nZcropUp+dx,dy+1:nX);
                    IMGout(:,:,iR)=As(1:fftScale:end,:);
                end
            end
			

function [output, Greg,Gfl1] = OCTA_F_dftreg(buf1ft,buf2ft,buf2ft1,usfac,Colshift)
...
if usfac == 0
    ...
    rfzero = sum(abs(buf1ft(:)).^2); % Ref
    rgzero = sum(abs(buf2ft(:)).^2); % Cur
...
%     Greg = Greg*exp(i*diffphase);
    % Flow
    Gfl1 = buf2ft1.*exp(1i*2*pi*(-row_shift*Nr/nr-col_shift*Nc/nc));
%      Gfl1=real(Gfl1);
%     Gfl1 =Gfl1*exp(i*diffphase);
    
elseif (nargout > 1)&&(usfac == 0)
%     Greg = buf2ft*exp(i*diffphase);
% %     Gfl1 = buf2ft1*exp(i*diffphase);
       Greg = buf2ft;
    Gfl1 = buf2ft1;
end
return
